...
import 'package:school_apps/screens/features/guru/sub_menu_quiz/soal_quiz/response/soalResponse.dart';

import '../../../../../../constants.dart';
import '../../../../../../main.dart';
import '../../../../../../size_config.dart';
import '../input_soal_screens.dart';

class FormInputSoalQuiz extends StatefulWidget {
  @override
  _FormInputSoalQuiz createState() => _FormInputSoalQuiz();
}

class _FormInputSoalQuiz extends State<FormInputSoalQuiz> {
  ...
  void initState() {
    ...
    namaSoalText = List.generate(
        InputSoalQuiz.data['jumlahSoal'], (i) => TextEditingController());
  }
  ...
  Widget build(BuildContext context) {
    print(InputSoalQuiz.data['idTugas']);
    if (InputSoalQuiz.data['typeSoal'] == "Essay") {
      jumlahSoal = InputSoalQuiz.data['jumlahSoal'];
    } else {
      jumlahSoal = 1;
    }
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Buat Soal ${InputSoalQuiz.data['typeSoal']}',
            style: TextStyle(
          ...
          DefaultButton(
            ...
            press: () {
              if (InputSoalQuiz.data['typeSoal'] == "Essay") {
                int length = InputSoalQuiz.data['jumlahSoal'];
                for (var i = 0; i < length; i++) {
                  dataSoal.add({
                    "soal": namaSoalText[i].text,
                    "id_tugas": '${InputSoalQuiz.data['idTugas']}',
                    "jumlahSoal": '${InputSoalQuiz.data['jumlahSoal']}',
                    "idTugas": '${InputSoalQuiz.data['idTugas']}',
                    "type": '${InputSoalQuiz.data['typeSoal']}'
                  });
                }
                SoalQuizResponse.inputSoalRes(dataSoal, context);
              } else {
                SoalQuizResponse.inputSoalFileRes(
                    namaSoalText[0].text,
                    filePath,
                    '${InputSoalQuiz.data['idTugas']}',
                    '${jumlahSoal}',
                    '${InputSoalQuiz.data['typeSoal']}',
                    context);
              }
            },
          ),
  ...
  _getTextWidgets(data) {
    ...
    } else if (InputSoalQuiz.data['typeSoal'] == "File") {
      for (var i = 0; i < data; i++) {
        list.add(SizedBox(height: 10));
        list.add(buildSoal(i));
        list.add(SizedBox(height: 10));
