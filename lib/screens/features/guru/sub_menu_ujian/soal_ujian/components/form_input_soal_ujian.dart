...
import 'package:school_apps/screens/features/guru/sub_menu_ujian/soal_ujian/response/soalResponse.dart';
...
class FormInputSoalUjian extends StatefulWidget {
  @override
  _FormInputSoalUjian createState() => _FormInputSoalUjian();
}

class _FormInputSoalUjian extends State<FormInputSoalUjian> {
  ...
  void initState() {
    ...
    namaSoalText = List.generate(
        InputSoalUjian.data['jumlahSoal'], (i) => TextEditingController());
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (InputSoalUjian.data['typeSoal'] == "Essay") {
      jumlahSoal = InputSoalUjian.data['jumlahSoal'];
    } else {
      jumlahSoal = 1;
    }
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Buat Soal ${InputSoalUjian.data['typeSoal']}',
            style: TextStyle(
          ...
          DefaultButton(
            ...
            press: () {
              if (InputSoalUjian.data['typeSoal'] == "Essay") {
                int length = InputSoalUjian.data['jumlahSoal'];
                for (var i = 0; i < length; i++) {
                  dataSoal.add({
                    "soal": namaSoalText[i].text,
                    "id_tugas": '${InputSoalUjian.data['idTugas']}',
                    "jumlahSoal": '${InputSoalUjian.data['jumlahSoal']}',
                    "idTugas": '${InputSoalUjian.data['idTugas']}',
                    "type": '${InputSoalUjian.data['typeSoal']}'
                  });
                }
                SoalUjianResponse.inputSoalRes(dataSoal, context);
              } else {
                SoalUjianResponse.inputSoalFileRes(
                    namaSoalText[0].text,
                    filePath,
                    '${InputSoalUjian.data['idTugas']}',
                    '${jumlahSoal}',
                    '${InputSoalUjian.data['typeSoal']}',
                    context);
              }
            },
          ),
  ...
  _getTextWidgets(data) {
    ...
    } else if (InputSoalUjian.data['typeSoal'] == "File") {
      for (var i = 0; i < data; i++) {
        list.add(SizedBox(height: 10));
        list.add(buildSoal(i));
        list.add(SizedBox(height: 10));
